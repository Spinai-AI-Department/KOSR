...
export function SignupPage() {
  ...
  const inputCls = "w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent";
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="flex-1 flex items-start justify-center p-8 overflow-y-auto">
        <div className="w-full max-w-md py-8">
          ...
          <div className="mb-6">
            <h2 className="text-2xl text-gray-900 mb-1">회원가입</h2>
            <p className="text-gray-500 text-sm">관리자 승인 후 로그인할 수 있습니다.</p>
          </div>
          ...
          {success ? (
            <div className="flex flex-col items-center gap-4 py-10 text-center">
              ...
              <div>
                <p className="text-lg text-gray-900 mb-1">신청이 완료되었습니다</p>
                <p className="text-sm text-gray-500">관리자 승인 후 로그인할 수 있습니다.</p>
              </div>
              <Link to="/login" className="mt-4 text-sm text-gray-900 underline">
                로그인 페이지로 이동
              </Link>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  <label className="block text-sm text-gray-700 mb-1">아이디 <span className="text-red-500">*</span></label>
                  <input required value={form.login_id} onChange={set("login_id")} placeholder="영문, 숫자 3자 이상" className={inputCls} />
                </div>
                <div>
                  <label className="block text-sm text-gray-700 mb-1">비밀번호 <span className="text-red-500">*</span></label>
                  <input required type="password" value={form.password} onChange={set("password")} placeholder="8자 이상" className={inputCls} />
                </div>
                <div>
                  <label className="block text-sm text-gray-700 mb-1">비밀번호 확인 <span className="text-red-500">*</span></label>
                ...
                <div className="col-span-2">
                  <label className="block text-sm text-gray-700 mb-1">소속 병원 <span className="text-red-500">*</span></label>
                ...
                <div className="col-span-2">
                  <label className="block text-sm text-gray-700 mb-1">역할 <span className="text-red-500">*</span></label>
                ...
                <div>
                  <label className="block text-sm text-gray-700 mb-1">진료과</label>
                  <input value={form.department} onChange={set("department")} placeholder="신경외과" className={inputCls} />
                </div>
                <div>
                  <label className="block text-sm text-gray-700 mb-1">전문분야</label>
                  <input value={form.specialty} onChange={set("specialty")} placeholder="척추" className={inputCls} />
                </div>
                <div className="col-span-2">
                  <label className="block text-sm text-gray-700 mb-1">면허번호</label>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-2.5 bg-gray-900 text-white rounded-lg text-sm hover:bg-gray-800 transition-colors disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {loading ? (
                  <>
